...
<section class="about-hero"
  style="--h:<?php echo $height_vh; ?>vh; --ov:<?php echo $overlay / 100; ?>; <?php echo $bg_url ? 'background-image:url(' . esc_url($bg_url) . ');' : ''; ?>">
...
if ($img180)
  $milk_variants[] = ['key' => '180', 'label' => $label180, 'img' => $img180];
$img90 = (int) get_field('img_90ml');
$label90 = trim((string) get_field('label_90ml')) ?: '90ml';
if ($img90)
  $milk_variants[] = ['key' => '90', 'label' => $label90, 'img' => $img90];
?>
<main class="container orioni-single">
  ...
  <div class="orioni-single__grid">
    ...
    <section class="orioni-info">
      ...
      <?php if ($short): ?>
        <p class="orioni-lead"><?php echo esc_html($short); ?></p><?php endif; ?>
      ...
      <div class="orioni-options">
        <?php if (!empty($pack_variants)): ?>
          ...
          <div class="o-group">
            ...
            <div class="o-values">
              <span
                class="o-chip is-active js-pack-weight"><?php echo esc_html($pack_variants[0]['weight'] ?: '—'); ?></span>
            </div>
          </div>
        <?php elseif (!empty($milk_variants)): ?>
          <div class="o-group">
      ...
      $fb = $ins = $ytb = '';
      if ($catalog = get_page_by_path('san-pham')) {
        $pid = $catalog->ID;
      ...
      <div class="orioni-share">
        ...
        <?php if ($fb): ?><a href="<?php echo esc_url($fb); ?>" target="_blank" rel="noopener noreferrer"
            aria-label="Facebook">🔵</a><?php endif; ?>
        <?php if ($ins): ?><a href="<?php echo esc_url($ins); ?>" target="_blank" rel="noopener noreferrer"
            aria-label="Instagram">🟣</a><?php endif; ?>
        <?php if ($ytb): ?><a href="<?php echo esc_url($ytb); ?>" target="_blank" rel="noopener noreferrer"
              ...
              <li class="orioni-card">
                <a class="orioni-card__link" href="<?php the_permalink(); ?>">
                  <div class="orioni-card__frame">
                    <?php if ($img)
                      echo wp_get_attachment_image($img, 'large', false, ['loading' => 'lazy']); ?>
                  </div>
      ...
      <ul class="oc-grid" data-collection-grid data-total="<?php echo (int) count($ids); ?>">
        <?php foreach ($ids as $idx => $aid):
          ...
          <li class="oc-item oc-it" data-idx="<?php echo (int) $idx; ?>">
            <a href="<?php echo esc_url($full ? $full[0] : wp_get_attachment_url($aid)); ?>" class="oc-link" data-zoom
