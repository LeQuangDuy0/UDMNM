...
$thong_tin_san_pham = get_page_by_path('tin-tuc/thong-tin-san-pham');
$current_id = get_queried_object_id();
?>
<div class="about-switch" aria-label="About tabs">
  <div class="about-switch__wrap">
    <?php if ($thong_cao_bao_chi): ?>
      <a class="about-switch__item <?php echo ($current_id === $thong_cao_bao_chi->ID) ? 'is-active' : ''; ?>"
        href="<?php echo esc_url(get_permalink($thong_cao_bao_chi->ID)); ?>">
        Thông cáo báo chí
      </a>
    <?php endif; ?>
...
$img_html = '';
if ($img_id) {
  $img_html = wp_get_attachment_image($img_id, 'large', false, ['loading' => 'lazy']);
} elseif (has_post_thumbnail()) {
  $img_html = get_the_post_thumbnail(get_the_ID(), 'large', ['loading' => 'lazy']);
} else {
